...
export const ArtifactReleasesSheet = ({ project, isOpen, onOpenChange, onRunStarted }: Props) => {
  ...
  const createRelease = async () => {
    ...
    try {
      const result = await actions.createRelease.mutateAsync({ version: normalized, ref: ref.trim() || undefined });
      toast({ title: 'Release build started', description: normalized });
